...
-->
              ...
              <td class="px-6 py-4 text-center">
                <button onclick="openEditModal('{{ $docente->codigoDocente }}', '{{ $docente->ID_Usuario }}')" class="editar-curso bg-blue-500 hover:bg-blue-700 text-white px-3 py-1 rounded flex items-center gap-2">
                  <!-- Nuevo SVG -->
                ...
                <form method="POST" action="{{ route('docentes.cambiarEstado', $docente->codigoDocente) }}" class="inline-block">
                  @csrf
                  <button type="submit"
                    class="bg-yellow-500 hover:bg-yellow-700 text-white px-3 py-1 rounded flex items-center gap-2"
                    onclick="return confirm('¿Está seguro que desea deshabilitar este docente?')">
                    <!-- SVG para 'Deshabilitar' -->
                ...
                <form method="POST" action="{{ route('docentes.cambiarEstado', $docente->codigoDocente) }}" class="inline-block">
                  @csrf
                  <button type="submit"
                    class="bg-green-500 hover:bg-green-700 text-white px-3 py-1 rounded flex items-center gap-2"
                    onclick="return confirm('¿Está seguro que desea habilitar este docente?')">
                    <!-- SVG para 'Habilitar' -->
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" class="w-5 h-5" stroke="currentColor"
                      viewBox="0 0 24 24" stroke-width="2">
